# MAINTENANCE row 87 battery: the runner's luaLongString level choice (tools/test/run-tests.mjs).
# The bar is MAINT-87-long_string_boundary_test.lua.
#
...
# MAINT-87-long_string_boundary_test.lua, its message is a syntax error, and no other file failed.
...
#
# Usage: py tools/test/mutate_maint87_long_string.py
import hashlib, os, re, subprocess, sys

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
RUNNER = os.path.join(ROOT, "tools", "test", "run-tests.mjs")
BAR = "MAINT-87-long_string_boundary_test.lua"
OLD = '  while ((content + "]").includes("]" + eq + "]")) eq += "=";\n'
NEW = '  while (content.includes("]" + eq + "]")) eq += "=";\n'
