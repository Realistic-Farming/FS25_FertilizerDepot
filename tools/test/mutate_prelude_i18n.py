...
Ported from SoilFertilizer for FertilizerDepot (MAINTENANCE row 63); the only change is
the runner's per-file status prefix ("ok " / "x ").
...
Run from tools/test:  py mutate_prelude_i18n.py  (ALONE, through the test lock)
"""
import hashlib
import os
import re
...
try:
    # Keep the console UTF-8 so a non-ASCII assertion name cannot raise on print.
...
TICK = "ok "
CROSS = "x "     # this runner prints "ok <file>" and "x <file>", not the SoilFertilizer glyphs
...
def bar_result(output):
    ...
    for line in output.splitlines():
        if BAR in line and (line.startswith(TICK) or line.startswith(CROSS)):
            # The prefix is a word here, not one glyph, so compare the whole prefix.
            return (TICK if line.startswith(TICK) else CROSS), line.strip()
    return None, None
